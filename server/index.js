...
app.get('/api/webrtc-config',(_req,res)=>{const iceServers=[{urls:process.env.STUN_URL||'stun:stun.l.google.com:19302'}];if(process.env.TURN_URL)iceServers.push({urls:process.env.TURN_URL,username:process.env.TURN_USERNAME||'',credential:process.env.TURN_CREDENTIAL||''});res.json({iceServers,provider:'DSS Flow internal WebRTC'});});
...
io.on('connection',socket=>{
  ...
  socket.on('join-meeting',async({roomName,media={}}={})=>{try{const[meeting]=await sql`SELECT id,organizer_id FROM meetings WHERE room_name=${roomName}`;if(!meeting)return;const[attendee]=await sql`SELECT 1 FROM meeting_attendees WHERE meeting_id=${meeting.id} AND user_id=${user.id}`;if(!attendee&&meeting.organizer_id!==user.id)return;const room=`meeting:${roomName}`;socket.data.meetingMedia={...(socket.data.meetingMedia||{}),[room]:{mic:Boolean(media.mic),camera:Boolean(media.camera)}};const connected=await io.in(room).fetchSockets();const existing=connected.filter(person=>person.id!==socket.id).map(person=>({id:person.id,userId:person.data.user?.id,name:person.data.user?.name||'Participant',avatarColor:person.data.user?.avatar_color,media:person.data.meetingMedia?.[room]||{mic:false,camera:false}}));await socket.join(room);socket.data.meetingRooms.add(room);const participant={id:socket.id,userId:user.id,name:user.name,avatarColor:user.avatar_color,media:socket.data.meetingMedia[room]};socket.emit('meeting-participants',existing);socket.to(room).emit('participant-joined',participant);}catch(error){console.error('Unable to join realtime meeting:',error.message)}});
  ...
  socket.on('meeting-media-state',({roomName,mic,camera})=>{const room=`meeting:${roomName}`;if(!socket.rooms.has(room))return;const media={mic:Boolean(mic),camera:Boolean(camera)};socket.data.meetingMedia={...(socket.data.meetingMedia||{}),[room]:media};io.to(room).emit('meeting-media-state',{socketId:socket.id,userId:user.id,...media})});
